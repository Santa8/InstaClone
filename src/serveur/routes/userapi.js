...
router.post("/register", async (req, res) => {
  ...
  if (error) {
    return res.status(400).send({ value: false, message: error.details[0].message });
  }

  // check if user already exist
  const emailExist = await User.findOne({ email: req.body.email });
  if (emailExist) {
    return res.status(400).send({ value: false, message: "Email already exist" });
  }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!validPass) {
      return res.status(400).send({ value: false, message: "Password is not correct" });
